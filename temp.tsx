...
export default function Home() {
  ...
  const [error, setError] = useState<string | null>(null); // State for current language - SET TO 'RU' AS DEFAULT

  const [language, setLanguage] = useState<LanguageKey>("RU");
  const T: TranslationSet = translations[language]; // Helper function to format progress message strings (like "1/2: ...")

  const formatProgress = (key: keyof TranslationSet) => {
    const message = T[key] as string; // For progress messages, split the step number from the description
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      setLoading(true);
      setError(null);
      setHighlightUrls([]); // Step 1: Uploading
          ...
          {loading && (
            <div className="bg-[#1e1e1e] p-8 mb-8 flex flex-col items-center text-center space-y-4 sony-card">
              ...
              <img
                ...
                alt="Nezuko Running Loader" // Nezuko is larger and centered
